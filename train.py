...
class Trainer:
    ...
    def train_epoch(self):
        ...
        print(f"Starting training for epoch {self.epoch + 1}")
        ...
        print(f'Epoch [{self.epoch + 1}/{self.h_params.num_epochs}], Loss: {avg_loss:.4f}, Time: {epoch_time:.2f} seconds')
    ...
    def train(self):
        ...
        for epoch in range(self.h_params.num_epochs):         
            # Train epoch
            self.train_epoch()
...
def parse_args():
    ...
    parser.add_argument("-c", "--config", help="configuration file (required)", default=f"conf\\resnet18_no_pretraining.yaml")
